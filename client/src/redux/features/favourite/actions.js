...
export const addToFavouriteRequest = (data, token, toast) => async () => {
    try {
        delete data._id;
        await axios.post('http://localhost:8080/favourite', data, { headers: { 'Authorization': `Bearer ${token}` } });
...
export const getFavouriteRequest = (token) => async (dispatch) => {
    try {
        ...
        let res = await axios.get('http://localhost:8080/favourite', { headers: { 'Authorization': `Bearer ${token}` } });
